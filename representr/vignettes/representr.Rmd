...
- id: xxxx2020predicting
  title: "Predicting Voter Party Registration from Linked Data with Error Propagation."
  author:
    - family: XXXX
      given: XXXX
  issued:
...
Before we perform prototyping to get a representative data set using `representr`, we must first perform record linkage to remove duplicates and resolve entities present in the dataset. In the absense of unique identifying information (like a social security number), we can use probabilistic methods to perform record linkage. These methods consist of clustering records to latent (or true and unknown) individuals in the dataset.

For the examples in this vignette, we will use `blink` [@blink], which is a Bayesian record linkage model [@steorts2015entity]. For this example, we will run only a small number of iterations, which will certainly not have converged. For a real example, we would want to let this model run longer.
